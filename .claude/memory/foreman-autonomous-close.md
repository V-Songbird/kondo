...
**How to apply:** Two jig commit guards bite on every feature commit here: a `shared/contract.ts` change must ship a `docs/adr/**` edit, and an `electron/main/workspace/` change must ship a `docs/domain.md` edit — plan the doc line before committing. The Bash tool mangles `\\` inside heredocs on this machine; write Python edit scripts to the scratchpad with the Write tool and run them. In `&&` chains, `grep -c` exits 1 on zero matches and silently stops the chain, so never put it before a commit. See [[kondo-fixture-run-recipe]] for the live check.
