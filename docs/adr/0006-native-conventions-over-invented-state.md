...
| MCP server (every scope) | The name in that project's `disabledMcpServers` in `~/.claude.json`: Claude's `/mcp` switch, one list per project whatever scope declared the server. A user-scope server is switched from each project's page, never globally. | Approval of a `.mcp.json` server — `enabledMcpjsonServers`, `disabledMcpjsonServers` and `enableAllProjectMcpServers`, in settings files — is read and never planned, because Claude gates it behind its own prompt. A declaration never moves between files. |
